...
## Phase 6 — Multi-Principal Registry & Dual-Node Consensus

**Duration:** 3–4 weeks
**Goal:** Enforce consensus across independent validators and support multi-principal agent identities to meet Schmidt proposal specifications.

### 6.1 Multi-Principal Agent Registry
Extend the database catalog schema to support agent ownership tracking and scope:
- `principal_id` (UUID): Unique cryptographic key identifying the principal (e.g. Aligned Institute, Oxford).
- `payment_address` (text): EVM-compatible USDC address on Arc L1.
- `scope_constraints` (JSONB): Action boundaries (max payment per transaction, authorized domains).
- `reputation_score` (float): Rolling performance reliability index.

### 6.2 Dual-Node Consensus Validation
To prevent single-org control over aligned results, the Alignment Coordinator executes in a dual-node active-active consensus topology:
- **Primary Node** and **Secondary Node** run the alignment pipeline concurrently (can be hosted on different institutional infrastructures).
- A Canonical Aligned State (CAS) is only finalized if both coordinator nodes produce matching output hashes.
- Disagreement between the nodes logs a `consensus_failure` event to `research_events`, halts settlement, and triggers escalation.

---

## Phase 7 — Three-Tier Sub-Agent Triad (Agent Resiliency)

**Duration:** 2–3 weeks
**Goal:** Encapsulate individual agent execution lifecycles to isolate failures and guarantee clean pipeline inputs.

Each registered clinical/domain agent is refactored from a single script to a structured sub-agent triad:
1.  **Runner:** Stateless, dedicated coroutine executing the data fetch (e.g. vital signs reading from chartevents).
2.  **Watcher:** Internal monitor checking Runner health and enforcing the agent's freshness TTL. Handles auto-recovery or respawn and escalates to the Overseer upon failure.
3.  **Verifier:** Gates publication to the coordinator. Asserts the output schema conforms to AgentOutput, checks data boundaries (e.g. HR in valid human ranges), and captures quality logs.

---

## Phase 8 — Continuous Operation Mode

**Duration:** 4–5 weeks
**Goal:** Shift mSignals from a query-driven batch loop to a persistent continuous monitoring runtime.

- **Scheduler:** Manages parallel fetch jobs running on per-agent TTL intervals (`dispatch_schedule` table).
- **Live CAS:** Stores the Canonical Aligned State as an active PostgreSQL record updated incrementally on new agent check-ins.
- **Change Detector:** Calculates the confidence delta shift from the previous CAS version. A shift above a threshold or a new registered conflict registers an alert entry in the `alerts` table.
- **Alert Ingestion:** Pushes alerts to the Next.js dashboard using Supabase Realtime websocket feeds.

---

## Phase 9 — Circle x402 Micropayments & ZKP Provenance

**Duration:** 6–8 weeks
**Goal:** Hook alignment quality to economic settlement on Arc L1 and integrate cryptographic proof of knowledge.

- **Payment Authorization Gate:** Evaluates CAS aggregate quality and assigns payout weight (Full / Reduced / Held / Refused).
- **Circle x402 micropayments:** Authorizes gasless USDC nanopayments batched via Circle Gateway.
- **ADP 8.5 ZKP Proofs:** Integrates zero-knowledge tokens (Halo2) generated by ADP PoK oracles to verify that token outputs were strictly derived from bounded context. Anchors the ZKP alongside the CAS hash on the Arc L1 ledger at settlement.
...
|---|---|---|---|
| 0 — MIMIC Data Foundation [COMPLETE] | 3–4 wks | Working agents on real ICU data | None |
| 1 — Conflict Detection Engine [COMPLETE] | 4–6 wks | Compound pattern detection + validation | None |
| 2 — Research Dashboard [COMPLETE] | 6–8 wks | Nurse + doctor views, demo-ready | None |
| 3 — Validation Study [NEXT PHASE] | 8–12 wks | Published paper, AUROC evidence | None |
...
| 6 — Multi-Principal & Dual-Node | 3–4 wks | Dual-node consensus Coordinator, Registry | None |
| 7 — Three-Tier Sub-Agent Triad | 2–3 wks | Runner/Watcher/Verifier agent lifecycle | None |
| 8 — Continuous Operation Mode | 4–5 wks | Autonomous scheduler, Live CAS, alerts | None |
| 9 — Micropayments & ZKP Provenance | 6–8 wks | Circle x402, Arc L1, ADP 8.5 ZKPs | None |
